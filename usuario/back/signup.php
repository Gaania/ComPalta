...
if (isset($_GET)){
    ...
    //validaciones
    if($clave!=$clave2){
        echo "<script>alert('ERROR: Las contraseñas no son iguales');window.location='';</script>";
        exit();
    }
    ...
    if ($res){
        ...
        if($num!=0){
            echo "<script>alert('ERROR: Este nombre de usuario ya está asociado a una cuenta.');window.location='';</script>";
            exit();
        }
    }
    ...
    if ($res){
        $num=mysqli_num_rows($res);
        if($num!=0){
            echo "<script>alert('ERROR: Este correo ya está asociado a una cuenta.');window.location='';</script>";
            exit();
        }
    }

    //guardar registro
    $query="INSERT INTO `usuario`(`nombre`, `clave`, `correo`, `imagen`, `fechaCreacion`) VALUES ('$nombre','$clave','$correo','','$fechacreacion')";
    $res=mysqli_query($conexion, $query);
    if (!$res){
        echo "<script>alert('ERROR: No se guardó su usuario. Vuelva a intentar.');window.location='';</script>";
        exit();
    }
